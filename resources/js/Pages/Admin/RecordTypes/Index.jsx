...
export default function Index({ rows }) {
    ...
    const columnDefs = useMemo(
        () => [
            ...
            { field: 'code', headerName: 'Mã loại', maxWidth: 160, editable: true },
            { field: 'name', headerName: 'Tên loại', flex: 1.4, editable: true },
    ...
    const isDraftReady = (row) => Boolean(row.code) && Boolean(row.name);
    ...
    const onCellValueChanged = async (params) => {
        ...
        if (data.isDraft) {
            ...
            try {
                const response = await window.axios.post('/admin/record-types', {
        ...
        try {
            await window.axios.patch(`/admin/record-types/${data.id}`, {
                field: colDef.field,
                value: newValue,
            });
        } catch (error) {
    ...
    const onDelete = async (id) => {
        ...
        try {
            await window.axios.delete(`/admin/record-types/${id}`);
    ...
    return (
        <AdminLayout title="Loại hồ sơ">
            <Head title="Loại hồ sơ" />
